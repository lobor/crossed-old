...
var ProgressFilledTrack_exports = {};
__export(ProgressFilledTrack_exports, {
  ProgressFilledTrack: () => ProgressFilledTrack
});
module.exports = __toCommonJS(ProgressFilledTrack_exports);
...
function ProgressFilledTrack(StyledProgressFilledTrack) {
  return (0, import_react.forwardRef)(
    //@ts-ignore
    ({ sx, ...props }, ref) => {
      const { valueWidth } = (0, import_ProgressContext.useProgress)("ProgressContext");
      let mysx = {};
      if (sx) {
        sx["w"] = `${valueWidth}%`;
        mysx = sx;

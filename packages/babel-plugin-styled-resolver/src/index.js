...
const {
  ...
  getStyleIds,
} = require('@crossed/react/lib/commonjs/resolver');

const {
  convertStyledToStyledVerbosed,
  convertSxToSxVerbosed,
} = require('@crossed/react/lib/commonjs/convertSxToSxVerbosed');
const {
  propertyTokenMap,
} = require('@crossed/react/lib/commonjs/propertyTokenMap');
const { stableHash } = require('@crossed/react/lib/commonjs/stableHash');
const {
  INTERNAL_updateCSSStyleInOrderedResolved,
} = require('@crossed/react/lib/commonjs/updateCSSStyleInOrderedResolved');
const {
  INTERNAL_updateCSSStyleInOrderedResolved:
    INTERNAL_updateCSSStyleInOrderedResolvedWeb,
} = require('@crossed/react/lib/commonjs/updateCSSStyleInOrderedResolved.web');

const IMPORT_NAME = '@crossed/core';
let configThemePath = [];
...
function getConfig(configPath) {
  ...
  const isConfigJSExist = fs.existsSync(
    path.join(process.cwd(), './crossed.config.js')
  );
  ...
  const isConfigTSExist = fs.existsSync(
    path.join(process.cwd(), './crossed.config.ts')
  );
